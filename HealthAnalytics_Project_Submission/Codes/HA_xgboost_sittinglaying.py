...
results=results[results['posture']!=1]
results=results[results['posture']!=2]
tests = pd.read_csv("Shweta_mstdaccelerometerfinal.csv")
tests = tests.sample(frac=1)
tests=tests[tests['posture']!=1]
tests=tests[tests['posture']!=2]
# data = np.asarray(results)
# test_data = np.asarray(tests)
data=results
test_data=tests
xlimit = 15
xtrain = data.iloc[:,[0,1,2,12,13,14]]
ytrain = data.iloc[:, xlimit]

xtest = test_data.iloc[:, [0,1,2,12,13,14]]
